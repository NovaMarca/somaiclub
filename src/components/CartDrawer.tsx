  ...
  return <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger asChild>
        <Button variant="ghost" size="icon" className="relative">
          ...
          {totalItems > 0 && <Badge className="absolute -top-2 -right-2 h-5 w-5 rounded-full p-0 flex items-center justify-center text-xs bg-accent text-accent-foreground">
              {totalItems}
            </Badge>}
        </Button>
      </SheetTrigger>
          ...
          {items.length === 0 ? <div className="flex-1 flex items-center justify-center">
              ...
              <div className="flex-1 overflow-y-auto pr-2 min-h-0">
                <div className="space-y-4">
                  {items.map(item => <div key={item.variantId} className="flex gap-4 p-3 bg-secondary/50 rounded-lg">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {item.variantTitle !== "Default Title" && <p className="text-xs text-muted-foreground">
                            {item.variantTitle}
                          </p>}
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t bg-background">
                ...
                <Button onClick={handleCheckout} className="w-full bg-accent text-accent-foreground hover:bg-accent/90" size="lg" disabled={items.length === 0 || isLoading}>
                  {isLoading ? <>
